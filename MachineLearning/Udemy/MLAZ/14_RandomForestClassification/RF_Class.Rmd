...
```{r}
social.rf <- randomForest(Purchased ~ ., training, nTree = 10)

# Predict on test set results
y_pred.rf <- predict(social.rf, test[-3])
head(y_pred.rf)
```
...
(cm <- table(test$Purchased, y_pred.rf))
...
plot.rf.model <- function(set) {
  ...
  y_pred.rf.grid <- predict(social.rf, set.grid, type = "class") # use grid to predict values w/ our model
  
  plot(set[,-3],
       main = c("Random Forest (", deparse(substitute(set), nlines = 1),
  ...
  contour(x1,x2, matrix(as.numeric(y_pred.rf.grid), length(x1), length(x2)), add = T) # add logistic regression "split" line
  
  points(set.grid, pch = 19, col = if_else(y_pred.rf.grid == 1, "springgreen3", "tomato"))
  points(set, pch = 19, col = if_else(set[,3] == 1, "green4", "red3"))
  
}

plot.rf.model(training)
plot.rf.model(test)
```

See that our classifier is indeed making better predictions than linear classifiers because it can curve. It's capturing more points in the correct regions than the linear classifiers. We are see our classifier creating red regions around those scattered red dots above and to the right of the red region, making it more flexible than the original decision tree. It didn't overfit to traning in some places, as we can see multiple red points incorrectly classified, but in the test set plot we can see red regions with no test points in them, indicating some bias to the traning data by the forest.

 # Classification conclusion.
 
 It looks like **Kernel SVM** would be the best bet for this business problem, as it had good out-of-sample accuracy and did not overfit.