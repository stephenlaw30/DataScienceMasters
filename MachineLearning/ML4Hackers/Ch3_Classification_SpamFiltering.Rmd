...
easyham_docs <- easyham_docs[1:500]

all_easyham <- sapply(easyham_docs, function(x) get_msg(paste(easyham_path, x, sep="")))
easyham_tdm <- get_tdm(all_easyham)
easyham_matrix <- as.matrix(easyham_tdm)
easyham_counts <- rowSums(easyham_matrix)

# create a data frame from these counts and make sure strings aren't factors
easyham_df <- data.frame(cbind(names(easyham_counts), as.numeric(easyham_counts)), stringsAsFactors = F)

# rename the columns and convert frequency back to numbers
names(easyham_df) <- c("term","frequency")
easyham_df %<>%
   mutate(frequency = as.numeric(frequency))

# calculate % of docs in which a given term in this matrix occurs
easyham_occurrence <- sapply(1:nrow(easyham_matrix),
  # pass through each term in the doc matrix and find cells w/ a positive element (a count > 0)
  # divide by total # of cols in the TDM/# of docs in the ham corpus
  function(i) {length(which(easyham_matrix[i,] > 0))/ncol(easyham_matrix)})

# calculate frequency of each word w/in entire corpus
easyham_density <- easyham_df$frequency/sum(easyham_df$frequency)

# add new cols occurence + density vectors to data frame
easyham_df <- easyham_df %>%
  mutate(density = easyham_density, 
         occurrence = easyham_occurrence)
```
```{r}
head(easyham_df[with(easyham_df, order(-occurrence)),])
```
...
classify_email <- function(path, training_df, prior = 0.5, c = 1e-6) {
  # get email message text, turn into TDM, and calculate frequency of terms
  msg <- get_msg(path)
  msg_tdm <- get_tdm(msg)
  msg_freq <- rowSums(as.matrix(msg_tdm))
  
  # Find intersections of words in new emails w/ training data
  msg_match <- intersect(names(msg_freq), training_df$term)
  
  # if we do not match a term, get the updated probability the email is NOT class in question
  if(length(msg_match) < 1) {
    return(prior * c^(length(msg_freq)))
  # if match, get occurence of matched term and update the prior probabilty email is class in question
  } else {
...
```
1st 3 steps of `classify.email` function proceed just as the training phase did = extract the message text w/ `get.msg`, turn it into a TDM w/ `get.tdm`, + finally calculate frequency of terms w/ `rowSums`. 
Next, need to find how the terms in the email message intersect w/ the terms in our training data

Final step of classification = determine whether any words in new email's message text are present in the training set, + if so, use them to calculate the probability the message is of the class in question

Assume for now we're attempting to determine if an email is spam. `msg.match` will contain all terms that are present in both the email message + in our spam training data, `spam.df`. 

If that intersection is empty, length of `msg.match` = 0 zero, + we can update our prior only by multiplying it w/ our tiny probability value `c` raised to the # of terms in the email. The result will be a tiny probability of assigning the spam label to the email.

Conversely, if this intersection is *not* empty, we need to find those terms from the email in our training data + look up their `occurrence` probabilities via `match` to do the lookup, which returns the term’s element position (index) in the term's column of our training data. 

Use these element positions (index) to return the corresponding probabilities from the `occurrence` column to `match.probs`, then calculate the product of these values + combine it w/ our prior belief about the email being spam w/ the term probabilities + the probabilities of any missing terms.

The result = our **Bayesian estimate for the probability a message is spam given the matching terms in our training data**.

As an initial test, use training data from `spam` + `easy ham` messages to classify `hard ham` emails. We know all of these emails are ham, so ideally our classifier will assign a higher probability of being ham to all of these messages. We also know, however, that hard ham messages = “hard” to classify b/c they contain terms that're also associated w/ spam. 
...
# classify hard ham docs with spam training data probabilites
# returns vector of conditional probability of spam for each email given the training data
hardham_spamtest <- sapply(hardham_docs,
                           function(p) classify_email(paste(hardham_path, p, sep=""),
                                         training_df = spam_df))

# classify hard ham docs with ham training data probabilites
# returns vector of conditional probability of ham for each email given the training data
hardham_hamtest <- sapply(hardham_docs,
                          function(p) classify_email(paste(hardham_path, p, sep=""),
                                          training_df = easyham_df))

# if the probability of an email being spam > being ham, classify as "success"
hardham_res <- ifelse(hardham_spamtest > hardham_hamtest, TRUE, FALSE)
table(hardham_res)
```
```{r}
(sensitivity <- 15/(234+15)) # true positive rate
```
Congratulations! You’ve written your first classifier, + it did fairly well at IDing hard ham as nonspam. In this case, we have approximately a 6% false-positive rate (6% of hard ham emails are incorrectly identified as spam).

A better test is to see how the classifier performs against not only hard ham, but also easy ham + spam.

# Testing the Classifier Against All Email Types

1st step = Build a simple function to do the the probability comparison from the previous section all at once for *all* emails.
```{r}
spam_classifier <- function(path) {
  
  # determine if email is spam (based on spam training data) or ham (based on easyham training data)
  # returns probabilities of spam + easy ham for each email
  prob_spam <- classify_email(path, spam_df)
  prob_ham <- classify_email(path, easyham_df)
  
  return(c(prob_spam, prob_ham, ifelse(prob_spam > prob_ham, 1, 0)))
}
```

Now test the 2nd sets of spam, easy ham, + hard ham using this simple classifier. 
...
# Classify them
spam2_class <- lapply(spam2_docs, function(p) {spam_classifier(file.path(spam2_path, p))})
easyham2_class <- lapply(easyham2_docs, function(p) {spam_classifier(file.path(easyham2_path, p))})
hardham2_class <- lapply(hardham2_docs, function(p) { spam_classifier(file.path(hardham2_path, p))})
```

```{r}
## Create a single, final, data frame w/ all classification data in it

# combine all rows from the list of classes for each email type
# returns 3 cols = prob of being given class, prob of NOT being givenclass, binary class var (T/F)
...
# combine these with the given class
...
# combine all 3 matrices above into final matrix and turn into dataframe
class_matrix <- rbind(easyham2_final, hardham2_final, spam2_final)
class_df <- data.frame(class_matrix, stringsAsFactors = FALSE)

# rename cols, combining hard + easy hams into 1 prob col
names(class_df) <- c("Prob_SPAM" ,"Prob_HAM", "Class", "Type")

# fix types
class_df %<>%
  mutate(Prob_SPAM = as.numeric(Prob_SPAM),
         Prob_HAM = as.numeric(Prob_HAM),
         Class = as.logical(as.numeric(Class)),
         Type = factor(Type))
head(class_df)
```
From the first 6 entries, it seems the classifier has done well, but let’s calculate the false-positive + false-negative rates for the classifier across all data sets by constructin an N×M matrix of our results, where rows = actual classification types + columns = predicted types. 

B/c we have 3 types of email being classified into 2 types, our confusion matrix will have 3 rows + 2 columns, w/ columns = % predicted as ham or spam, which if our classifier works perfectly, will read [1,1,0] and [0,0,1], respectively.
...
# get % of each class that was predicted FALSE (ham) and TRUE (spam)
...
class_cm <- rbind(easyham2_col, hardham2_col, spam2_col)
colnames(class_cm) <- c("NOT SPAM", "SPAM")
class_cm
```

Unfortunately, we did not write a perfect classifier, but the results are still quite good. Similar to our initial test, we get about a 1.5% and 6% false-positive rates, w/ the classifier doing slightly better on easy ham than hard. The false-negative rate is much higher at 26%. 

To get a better sense of how our classifier fared, we can plot the results using a scatterplot, w/ predicted probabilities of being ham on the x-axis + of spam on the y-axis.

```{r}
# Create final plot of results
(class_plot <- ggplot(class_df, aes(x = log(Prob_HAM), log(Prob_SPAM))) +
...
```
This scatterplot is in **log-log** scale b/c many predicted probabilities are very tiny, while others are not. W/ this high degree of variance, it is difficult to compare results directly, so taking logs is a simple way of altering the visual scale to more easily compare values.

We have also added a simple decision boundary to the plots where y = x, the **perfect linear relationship**, b/c our classifier compares the 2 predicted probabilities + determines an email’s type based on whether probability of being spam is greater than that of ham. All dots above the black diagonal line, therefore, *should* be spam, + all those below should be ham. 

This is not the case, but there is considerable clustering of message types as both probabilities rise. Also there is some intuition about how the classifier is underperforming w/ respect to false-positives. There appear to be 2 general ways it is failing. 
<ul>
<li> There are many hard ham messages that have a positive probability of being spam but near-zero probability of being ham = points pressed up against the y-axis.</li>
<li> There are both easy + hard ham messages that have a much higher relative probability of being ham. </li>
</ul>

Both of these observations may indicate a weak training data set for ham emails, as there are clearly many more terms that should be associated w/ ham that currently are not.

# Improving the Results
For a basic idea of  text classification, we constructed a very simple **Naive Bayes classifier** using a minimal number of assumptions + features. 

At its core, this type of classification is an application of classic conditional probability theory in a contemporary context. Despite the fact we trained our classifier w/ only a fraction of the total available data (1st 500 files in `ham`), this simple method performed
reasonably well.

That said, the false-positive + false-negative rates found in the test data are far too
high for any production spam filter. There are many simple tweaks we could apply to improve the results of the current model.

For example, our approach assumes a priori that each email has an equal probability of being ham or spam. In practice, however, we know that this relationship is actually much closer to 80%–20% ham-to-spam. 

1 way we might improve results would be to simply **alter our prior beliefs** to reflect this fact and recalculate the predicted probabilities.
```{r}
# alter priors to better reflect reality of 80/20 ham/spam
spam_classifier_real <-function(path) {
  prob_spam <- classify_email(path, spam_df, prior = 0.2)
  prob_ham  <- classify_email(path, easyham_df, prior = 0.8)
 return(c(prob_spam, prob_ham, ifelse(prob_spam > prob_ham, 1, 0)))
}
```

Remember we left the `prior` parameter as something that could vary in `classify_email()`. We could rerun the classifier now + compare the results.
```{r, echo = F}
# Classify emails with new classifier
spam2_class2 <- lapply(spam2_docs, 
                      function(p) {spam_classifier_real(file.path(spam2_path, p))})
easyham2_class2 <- lapply(easyham2_docs, 
                         function(p) {spam_classifier_real(file.path(easyham2_path, p))})
hardham2_class2 <- lapply(hardham2_docs, 
                         function(p) { spam_classifier_real(file.path(hardham2_path, p))})

## Create a single, final, data frame w/ all classification data in it

# combine all rows from the list of classes for each email type
# returns 3 cols = prob of being given class, prob of NOT being givenclass, binary class var (T/F)
spam2_matrix2 <- do.call(rbind, spam2_class2)
easyham2_matrix2 <- do.call(rbind, easyham2_class2)
hardham2_matrix2 <- do.call(rbind, hardham2_class2)

# combine these with the given class
spam2_final2 <- cbind(spam2_matrix2, "SPAM")
easyham2_final2 <- cbind(easyham2_matrix2, "EASYHAM")
hardham2_final2 <- cbind(hardham2_matrix2, "HARDHAM")

# combine all 3 matrices above into final matrix and turn into dataframe
class_matrix2 <- rbind(easyham2_final2, hardham2_final2, spam2_final2)
class_df2 <- data.frame(class_matrix2, stringsAsFactors = FALSE)

# rename cols, combining hard + easy hams into 1 prob col
names(class_df2) <- c("Prob_SPAM" ,"Prob_HAM", "Class", "Type")
...
# get % of each class that was predicted FALSE (ham) and TRUE (spam)
spam2_col2 <- get_results(subset(class_df2, Type == "SPAM")$Class)
easyham2_col2 <- get_results(subset(class_df2, Type == "EASYHAM")$Class)
hardham2_col2 <- get_results(subset(class_df2, Type == "HARDHAM")$Class)

# create confusion matrix
class_cm2 <- rbind(easyham2_col2, hardham2_col2, spam2_col2)
colnames(class_cm2) <- c("NOT SPAM", "SPAM")
class_cm2
```

This has slightly improved our easy ham and spam classifications, but not our hard ham classifications.

```{r, echo = F}
# Create final plot of results
(class_plot2 <- ggplot(class_df2, aes(x = log(Prob_HAM), log(Prob_SPAM))) +
    geom_point(aes(shape = Type, alpha = 0.5)) +
    geom_abline(intercept = 0, slope = 1) +
    scale_shape_manual(values = c("EASYHAM" = 1,
                                  "HARDHAM" = 2,
...
These new assumptions, however, violate the distributions of ham and spam messages in our training data. To be more accurate, we should go back and retrain the classifier with the *complete* easy ham data set (we limited our original ham training data to only the 1st 500 messages so that our training data would reflect the Bayesian assumptions). 

In that vein, we must incorporate the full data set in order to reflect our new assumptions.
When we rerun the classifier with the new easyham.df and classify.email parameterization,
we see a notable improvement in performance on false-positives
...
## re-create function w/ full easyham 
# + alter priors to better reflect reality of 80/20 ham/spam
spam_classifier_real <-function(path) {
  prob_spam <- classify_email(path, spam_df, prior = 0.2)
  prob_ham  <- classify_email(path, easyham_df, prior = 0.8)
...
With these simple changes, we have reduced our false-positive rate again (down to 1.2% and 4.4%, but false-negative results suffer (up to 29%). 

In essence, what we are doing is moving the decision boundaries and are explicitly trading off false positives for improvement in false negatives. This is an excellent example of why model specification is critical, and how each assumption and feature choice can affect all results.

Often it is difficult/impossible to classify observations based on a *single* decision boundary.