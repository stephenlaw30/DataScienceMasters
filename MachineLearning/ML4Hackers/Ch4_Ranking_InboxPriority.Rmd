...
```
Using the threads.matrix we just created, we will go back into the training data to find
all of the emails inside each thread. To do this, we create the get.threads function,
which will take the threads.matrix and our training data as arguments. Using the
unique command, we create a vector of all thread subjects in our data. Now we need
to take this information and measure each thread’s activity.
114 | Chapter 4: Ranking: Priority Inbox
The thread.counts function will do this. Using the thread subject and training data as
