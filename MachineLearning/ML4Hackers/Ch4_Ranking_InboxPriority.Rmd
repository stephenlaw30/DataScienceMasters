...
get_sender <- function(msg_vec) {
  # search for pattern in msg_vec + return line that matches
  # grepl, rather than returning vector *indices* like grep, returns a vector of the same length as
  #   the input w/ *Boolean* values
  from <- msg_vec[grepl(pattern="From: ", x = msg_vec)] 
  
  # split on colons, <>'s, + spaces to return list of components in "From" line
  # This pattern always put the address as 1st element i#n list, so pull w/ [[1]]. 
  from <- strsplit(from, '[":<> ]')[[1]] 
  
  # remove the empty spaces returned due to variation in patterns
  from <- from[which(from !="" & from !=" ")] 
  
  # find list elements that contains "@: and return it
...
get_msg <- function(msg_vec) {
  # start email body extraction after 1st line break up to end of msg
  msg <- msg_vec[seq(which(msg_vec == "")[1] + 1, length(msg_vec), 1)] 
...
get_subject <- function(msg_vec) {
  # find line that starts w/ "Subject" and extract it
...
sender_weight <- transform(sender_weight, Weight = log(Freq + 1), log10Weight = log10(Freq + 1))
head(sender_weight)
```
...
To generate a priority rank for each message in training data, we multiply all weights produced above. So, for each email, we parse the message, extract the features, + then match them to the corresponding weight data frames to get the appropriate weighting value. 

We then take the product of these values to produce a single—and-unique rank value for each message. `rank_message` will take a file path of a message + produce a priority ranking for that message based on the defined features + their subsequent weights. 

It relies on many functions already defined, as well as a new function, `get_weights`, which does the weight lookup when a feature does not map to a single weight (i.e., subject and message terms). It takes 3 args: some search terms (a string), the weight data frame in which to do the lookup, + a single Boolean value for `term`, which allows us to tell the application whether it is doing a lookup on a *term* data frame or on a *thread* data frame (lookups treated slightly differently due to differences in column labels in `thread_weights`). We use `match` to find elements in the given weight data frame that match `search_term` and return this term's weight value. 

For non-matches, 1st, we do 1 safety check to make sure the search term being passed to `get_weights` is valid by checking if it has some positive length (same type of check in parsing email data to check if an email had a subject line). If it is an invalid search term, simply return a 1 (which will not alter the product computed in the next step).

Next, `match` will return an `NA` for any elements in the search term vector that do not match `search_term` so we'd only extract the weight values for those matched elements that are not `NA`. If there are no matches, the `term_match` vector will be all `NA`, in which case `match_weights` will have a "zero length". When doing an additional check for this case, and if it is encountered, we again return 1. If we *have* matched some weight values, we return the mean of all these weights as our result.

```{r}
get_weights <- function(search_term, weight_df, term = TRUE) {
    if(length(search_term) > 0) {
      # if we are given a term to search for
      if(term) {
        # if given search term is a weighted term, get that those indices
        term_match <- match(names(search_term), weight_df$Term)
      } else {
        # if not, and if given search term is in a weighted term, get those indices
        term_match <- match(search_term, weight_df$Thread)
      }
      # get the Weights of the returned indices in the dataframe from above 
      match_weights <- weight_df$Weight[which(!is.na(term_match))]
      
      # if we have indeed found weights
...
rank_message <- function(path) {
  ...
  # 1st Weighting = just on total frequency of a sender's email address
  
  # if we found a match of a sender's email adress, get that sender's weight
  from <- ifelse(length(which(sender_weight$From_EMail == msg[2])) > 0,
        sender_weight$Weight[which(sender_weight$From_EMail == msg[2])], 1)
  
  # 2nd Weighting = based on senders in threads + threads themselves
  
  # if a sender's email if is our activity data frame, get that activity weight
  thread_from <- ifelse(length(which(sender_activity$From_EMail == msg[2])) > 0,
        sender_activity$Weight[which(sender_activity$From_EMail == msg[2])], 1)
  
  # extract subject from messages 
  subj <- strsplit(tolower(msg[3]), "re: ")
  # confirm is subject is part of a a thread
  is_thread <- ifelse(subj[[1]][1] == "", TRUE, FALSE)
    # if part of thread, get the weight of the terms in the thread and average them
    if(is_thread) {
    ...
    # get the terms from the subject of the message + get the weights of those terms if they're in threads
    thread_terms <- get_terms(msg[3], control=list(stopwords=stopwords()))
    thread_terms_weights <- get_weights(thread_terms, term_weights)
    
    # Weight based terms in all messages
    # get terms from the emails body + get their weights
    msg_terms <- get_terms(msg[4], control=list(stopwords=stopwords(), removePunctuation=TRUE,
                                                removeNumbers=TRUE))
    msg_weights <- get_weights(msg_terms, msg_weights)
    
    # Calculate rank by multiplying all weights
    weights <- as.numeric(c(from, thread_from, activity, thread_terms_weights, msg_weights))
    rank <- prod(weights)
    return(c(msg[1], msg[2], msg[3], rank))
}
```

We are now ready to fire up our ranker by splitting our data into 2 chronologically divided sets
```{r}
# split easy ham data frame in half and use 1st half as training and 2nd half as test
train_paths <- priority_df$Path[1:(round(nrow(priority_df) / 2))]
test_paths <- priority_df$Path[((round(nrow(priority_df) / 2)) + 1):nrow(priority_df)]
```

We will use the ranking data generated from here to establish a threshold value for a “priority” message. Once we have this, we can run the ranker over the emails in `test.paths` to determine which ones are priority + to estimate their internal rank ordering. 

```{r}
# rank each message in training data and convert resulting list to matrix
train_ranks <- suppressWarnings(lapply(train_paths, rank_message))
train_ranks_matrix <- do.call(rbind, train_ranks)

# add marker column to note this is training data + add its path, then convert to data frame
train_ranks_matrix <- cbind(train_paths, train_ranks_matrix, "TRAINING")
train_ranks_df <- data.frame(train_ranks_matrix, stringsAsFactors=FALSE)
names(train_ranks_df) <- c("Message", "Date", "From", "Subj", "Rank", "Type")
train_ranks_df$Rank <- as.numeric(train_ranks_df$Rank)

# get median rank to create threshold for prioritizing test data
priority_threshold <- median(train_ranks_df$Rank, na.rm = T)
train_ranks_df$Priority <- ifelse(train_ranks_df$Rank >= priority_threshold, 1, 0)
```

Because we are not using pre-existing examples of how emails ought to be ranked to determine this threshold, we are performing a task that is not really a "standard" sort of supervised learning. But we have chosen the median as our threshold for 2 principled reasons. 
<ul>
<li> If we have designed a good ranker, the ranks should have a smooth distribution, w/ most emails having low rank + many fewer emails having a high rank. Since we're looking for “important emails,” (those that're most unique or unlike the normal flow of email traffic), those will be the emails in the right tail of the rank distribution. If this is the case, those values greater than the median will be those that are somewhat greater than the typical rank. Intuitively, this is how we want to think about recommending priority email, those with larger ranks than the typical email. </li>
<li> We know the test data will contain email messages that have data that does not match anything in our training data. New emails are flowing in constantly, but given our setup, we have no way of updating our ranker. As such, we may want to have a rule about priority that is more inclusive than exclusive. If not, we may miss messages that are only partial matches to our features. As a final step, we add a new binary column `Priority` to `train_ranks_df`, indicating whether the email will be recommended as priority by our ranker. </li>
</ul>

```{r}
# Visualize the results to locate threshold
(threshold_plot <- ggplot(train_ranks_df, aes(x = Rank)) +
  stat_density(aes(fill="darkred")) +
  geom_vline(xintercept = priority_threshold, linetype = 2) +
  scale_fill_manual(values = c("darkred" = "darkred"), guide = "none") +
  theme_bw())
```

This is shows the density estimate for the ranks calculated on our training data. The vertical dashed line is the median threshold, which is about 6.5. Our ranks are very heavy-tailed, so we have created a ranker that performs well on the training data. We can also see that the median threshold is very inclusive, with a large portion of the downward-sloping density included as priority email. Again, this is done intentionally. A much less inclusive threshold would be to use the standard deviation of the distributions, which we can calculate.
```{r}
sd(train_ranks_df$Rank, na.rm = T)
```

The standard deviation is about 17, which would almost exactly exclude any emails outside of the tail.

We will now calculate the rank values for all of the emails in our test set. 
```{r}
# rank each message in testing data and convert resulting list to matrix
test_ranks <- suppressWarnings(lapply(test_paths, rank_message))
test_ranks_matrix <- do.call(rbind, test_ranks)

# add marker column to note this is testing data + add its path, then convert to data frame
test_ranks_matrix <- cbind(test_paths, test_ranks_matrix, "TESTING")
test_ranks_df <- data.frame(test_ranks_matrix, stringsAsFactors=FALSE)
names(test_ranks_df) <- c("Message", "Date", "From", "Subj", "Rank", "Type")
test_ranks_df$Rank <- as.numeric(test_ranks_df$Rank)
test_ranks_df$Priority <- ifelse(test_ranks_df$Rank >= priority_threshold, 1, 0)
```

Now we can compare the results and see how well our ranker did on new emails.

```{r}
final_df <- rbind(train_ranks_df, test_ranks_df)
final_df$Date <- date_converter(final_df$Date, pattern1, pattern2)
final_df <- final_df[rev(with(final_df, order(Date))), ]

# Save final data set and plot results_
#write_csv(final_df, file_path("data", "final_df_csv"), row_names = FALSE)

(testing_plot <- ggplot(subset(final_df, Type == "TRAINING"), aes(x = Rank)) +
  stat_density(aes(fill = Type, alpha = .65)) +
  stat_density(data = subset(final_df, Type == "TESTING"),
               aes(fill = Type, alpha = .65)) +
  geom_vline(xintercept = priority_threshold, linetype = 2) +
...
Notice there is much more density in the test data at the very low end of the distributions, which means there are many more emails with a low rank. Additionally, the test density estimate is much less smooth than the training data, which is evidence that the test data includes many observations not in our training data. B/c these observations do not match anything in our training data, the ranker effectively ignores this info.

Although this is problematic, we avoid disaster b/c we used an inclusive threshold for priority email. There is still a reasonable amount of density for the test data to the right of the threshold line which means our ranker was still able to find emails to recommend as important from the test data. 

There is an inherent “unknowable” quality to creating a ranker of this type. Throughout this entire exercise, we have posited assumptions
about each feature we included in our design + attempted to justify these intuitively. However, we can never know the “ground truth” as to how well our ranker is doing, because we can’t go back + ask the user for whom these emails were sent whether the ordering is good or makes sense. In the classification exercise, we knew the labels for each email in the training + test set, so we could measure explicitly how well the classifier was doing using the confusion matrix. In this case we can’t, but we can try to infer how well the ranker is doing by looking at the results. This is what makes this exercise something distinct from standard supervised learning.

As a final check, let’s actually see which emails our ranker pushed to the top 
```{r}
test_ranks_df %>%
  filter(Priority == 1) %>%
  select(Date, From, Subj, Rank) %>%
  head(30)
```
