...
```
  ...
  * They are virtually identical, with a lower $\hat{\beta_0}$ and *slightly* lower $\hat{\beta_1}$, which can be rounded to match $\beta_1$
...
```
  ...
  * Our $\hat{\beta_0}$ has slightly increased from 2 to 2.1305, while our $\hat{\beta_1}$ has decreased by over .5 from 2 down to 1.4396, while $\hat{\beta_3}$ has increased from .3 to 1.0097. We cannot reject the null $H_02: \beta_2 = 0$, but we *may* be able to reject the null $H_01: \beta_1 = 0$, depending on our threshold for doing so (is 1 `*` enough?)

**(d) Now fit a least squares regression to predict $y$ using only $x_1$. Comment on your results. Can you reject $H_01: \beta_1 = 0$?**
```{r}
lm_14_2 <- lm(y~x1)
summary(lm_14_2)
```

* Now we can reject $H_01: \beta_1 = 0$

**(e) Now fit a least squares regression to predict $y$ using only $x_2$. Comment on your results. Can you reject the null H0 : B1 = 0?**
```{r}
lm_14_3 <- lm(y~x2)
summary(lm_14_3)
```

* Now we can reject $H_02: \beta_2 = 0$
...
```
  ...
  * It now appears that we can see some individual effect from $x_2$ in the multiple linear regression model (as opposed to x1 in the original multiple regression), while both individual effects are still clearly visible in the simple models.
...
```
  ...
  * The variables for which we can reject the null $H_{0j} = \beta_j = 0$ are `rad`, `dis`, less so `medv`, and less so `zn`, `black`. So, looking at the coefficients, we can see that an increase of 1 unit in the index of accessibility to radial highways would increase per capita crime rate by .588 units, an increase of 1 unit in the weighted mean of distances to five Boston employment centres would decrease per capita crime rate by .987 units, and an increase in in median home value of $1K would mean a decrease of .198 units for crime rate.
