...
* Take the log of the odds give the **logit** = `ln(p/(1-p))` (4.4). Plugging into 4.3 gives `ln[p(X)(1-p(X))] = B0 + B1\*X`. 
* From 4.3: `p(X)(1-p(X)) = exp(B0 + B1\*X)`, we have `p(X) = exp(B0 + B1\*X)*(1-p(X)) =  exp(B0 + B1\*X)*(1-exp(B0 + B1\*X)  )`
* ????????????????????????????????????????

**2. Classifying an observation to the class for which (4.12) [p_k(x)] is largest is equivalent to classifying an observation to the class for which (4.13) [sigma_k(x)] is largest. Prove that this is the case. In other words, under the assumption the observations in the kth class are drawn from a N(mu_k, simga^2) distribution, the Bayes' classifier assigns an observation to the class for which the discriminant function is maximized.**
