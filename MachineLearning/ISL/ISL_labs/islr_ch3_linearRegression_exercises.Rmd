...
```
  ...
  * **ii. How strong is the relationship between the predictor and the response?**
    * The low $p$-value and \*\*\* significance level indicates evidence of a relationship, and the moderate $R^2$ value indicated a moderately strong relationship, which we can look at with a scatterplot and correlation calculation
...
```
  ...
  *********fdskjangakjgnhasdkljnghakjfd We have 95% confidence the interval `[23.97308, 24.96108]` contains the true parameter value for the predicted `mpg` value for a value of 98 for `horsepower` (range for expected value of $y$ given a value for $x$, $E[y|x]$)
  * We have 95% confidence the interval `[14.8094, 34.12476]` contains the true parameter value for the predicted `mpg` value for a value of 98 for `horsepower` (range for expected value of $y$ itself, $E[y]$)
...
* **(d) Use `plot()` to produce diagnostic plots of the linear regression fit. Comment on any problems you see with the fit. Do the residual plots suggest any unusually large outliers? Does the leverage plot identify any observations with unusually high leverage?**
```{r}
par(mfrow=c(2,2))
plot(lm2)
```
...
**(c) Write out the model in equation form, being careful to handle the qualitative variables properly.**
  * $y = 13.043 - .054Price - .024Urban + 1.2US$
...
**11. In this problem we will investigate the t-statistic for the null hypothesis $H_0: \beta = 0$ in simple linear regression without an intercept. To begin, generate a predictor x and a response y as follows.**
```{r}
set.seed(1)
x <- rnorm(100)
y <- 2*x+rnorm(100)
...
**(d) For the regression of $Y$ onto $X$ without an intercept, the t-statistic for $H_0: \beta = 0$takes the form $\frac {\hat{\beta}} {SE(\hat{\beta})}$, where $\hat{\beta}$ = given by (3.38), and where Std. Error has its own formula. (slightly different from Sections 3.1.1 + 3.1.2, since here we're performing regression w/out an intercept.) Show algebraically, + confirm numerically in R, that the $t$-statistic can be written as**
```{r}
...
**(a) Recall the coefficient estimate $\hat{\beta}$ for linear regression of $Y$ onto $X$ w/out an intercept is given by (3.38). Under what circumstance is the coefficient estimate for the regression of $X$ onto $Y$ the same as the coefficient estimate for the regression of $Y$ onto $X$?**
...
```
* It appears that indus, nox, dis, medv, less so, ptratio, and (barely) age have a non-linear relationship with crimtitle: "Untitled"
author: "Steve Newns"
date: "May 18, 2018"
output: html_document
---
