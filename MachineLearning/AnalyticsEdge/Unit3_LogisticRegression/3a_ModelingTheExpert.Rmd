...
So, by increasing t, our Specificity/Recall increased while Sensitivity decreased, meaning we have less True Positives and/or more True Negatives.
...
So, by increasing t, our Specificity/Recall decreased while Sensitivity increased, meaning we have more True Positives and/or less True Negatives.

### Decisions

Which threshold should we pick?
  - Picking a good threshold value is often challenging.
  - Receiver Operator Characteristic (ROC) curve can help decide which t value is best.
  - Sensitivity/TP rate of the model is shown on Y + FP Rate (i.e. 1 - specificity/TN Rate) is on X
  - A line shows how these 2 outcome measures vary w/ different threshold values.
An ROC curve always starts at (0, 0), which corresponds to t = 1.
  - If you have t = 1, you will not catch ANY true (poor) cases --> sensitivity of 0.
  - But you WILL correctly label of ALL false (good) cases --> get a FPR = 0 (TN = 1)
ROC curve always ends at (1,1) which corresponds to a t value = 0
  - If you have a t = 0, you catch ALL TRUE (poor) cases) --> a sensitivity = 1 
  - but label all FALSE (good) cases as TRUE (poor) care cases too, meaning you get a FPR = 1 (TN = 0)
-T decreases as you move from (0,0) to (1,1).
   Ex: At (0, 0.4), you're correctly labeling about 40% of true/poor cases w/ a very small FPR (med Y, low X)
   On the other hand, @ (0.6, 0.9), you're correctly labeling about 90% of true/poor cases, but w/ a FPR = 60%.
   In the middle, around (0.3, 0.8), you're correctly labeling about 80% of true/poor cases, w/ a 30% FPR

The ROC curve captures ALL THRESHOLDS SIMULTANEOUSLY.
   - The higher the t value/closer to (0, 0) ==> the higher the SPEC and the lower the SENS
   - The lower the t value/closer to (1,1) ==> the higher the SENS and lower the SPEC

Should select the best threshold for the TRADE-OFF YOU WANT TO MAKE.
   - If more concerned w/ having a high specificity/low FPR, pick the threshold that maximizes the TPR while keeping 
       FPR really low (COST OF FAILING TO DETECT POSITIVES)
           - A threshold around (0.1, 0.5) on this ROC curve looks like a good choice in this case.
   - On the other hand, if more concerned w/ having a high sensitivity/high TPR, pick a threshold that minimizes FPR 
       but has a very high TPR (COST OF RAISING FALSE ALARMS)
...
library(ROCR)
Use out training set predictions to create the ROC curve w/ the prediction() function from lib ROCR
   - arg1 = the predictions from model, arg2 = TRUE outcomes of our data points, arg3 = label ordering
ROCRpred <- prediction(qualityPredTrain, qualityTrain$PoorCare)
now use performance() to define what to plot on X and Y of the ROC curve
   - arg1 = a prediction object output (see above), x-axis, y-axis)
ROCRperf <- performance(ROCRpred,measure = "tpr",x.measure = "fpr")
Plot this output w/ colors + have t-values go from 0 to 1 in increments of 0.1 
plot(ROCRperf,colorize=TRUE,print.cutoffs.at=seq(0,1,0.1),text.adj=c(-0.2,1.7))
W/ this curve, we can determine which threshold value we want to use depending on our preferences as a decision-maker.
   - Want to ID most critical patients w/ high confidence --> t = 0.7
   - want to ID 1/2 of poor patients correctly w/ as few errors as possible --> t = 0.3

1 of the things we should look for is that there might be multicollinearity in our model.
   - The various IV's are correlated, + this might confuse the CoE's/betas in the model.
Tests to address this involve checking the correlations of IV's
   - If they're excessively high, this means there might be multicollinearity, + you have to potentially revisit the 
       model, 
   - Do the signs of the betas make sense. Is the coefficient beta positive or negative?
       -If the sign agrees w/ intuition, then multicollinearity isn't a problem, but if intuition suggests a different 
           sign, this might be a sign of multicollinearity.

So how do we interpret the results + understand whether we have a good model or not?
 Area Under the Curve (AUC) shows an ABSOLUTE MEASURE OF QUALITY OF PREDICTION-- in this particular case, 0.775 = 77.5%
   - Given that the perfect score = 100% + a 50% score = pure guessing (w/ 50% rate of success),
So AUC gives an absolute measure of quality, + is less affected by various benchmarks.
It illustrates how accurate the model is on a more absolute sense.
 - The maximum possible AUC = a perfect prediction = 1.0 
 - AUC = 0.5 --> it's pure guessing.

Other outcome measures that are important for us to discuss is the confusion matrix.
           Predict0 Predict1
   Actual0   TN      FP      
   Actual1   FN      TP
Accuracy = (TP + TN)/n             Error Rate = (FP + FN)/n
Sensitivity/TPR = TP/(TP + FN)     FN Error Rate = FN/(TP + FN)  or 1 - FNR
Specificity/TNR = TN/(TN + FP)     FP Error Rate = FP/(TN + FP)  or 1 - TPR

Sensitivity =  of times we predict TRUE + it is indeed TRUE
Specificity =  of times we predict FALSE + it is indeed FALSE

So for quality of care, just like in linear regression, we want to make predictions on a test set to compute 
   out-of-sample metrics.

We develop a logistic regression model using data, but would like to make predictions out-of-sample.
In our test set, we have 32 cases + make statements about the quality of a prediction out-of-sample w/:
qualityPredTest <- predict(qualLogistic, type = "response", newdata = qualityTest)
summary(qualityPredTest)
table(qualityTest$PoorCare,qualityPredTest > 0.3)
     FALSE TRUE
 0    19    5        --> 24 cases are ACTUALLY GOOD
 1     2    6        --> 8 cases are ACTUALLY POOR

21 PREDICTED GOOD, 11 PREDICTED POOR

So we've got 19 correct good cases, 6 correct poor cases, 5 FN's, and 2 FP's
Sens/TPR = 6/(6+2) = 6/8 = 0.75
Spec/TNR = 19/(19+5) = 19/24 = 0.791     
Accuracy = (19+6)/32 = 25/32 = 0.78125

Compute the test set predictions in R using original qualLogistic (Office Visits + Narcotics)
model1_predictTest = predict(qualLogistic, type="response", newdata=qualityTest)
Compute test set AUC w/ prediction(the model's predictions on test, actual DV outcomes of our test data)
ROCRpredTest = prediction(model1_predictTest, qualityTest$PoorCare)
auc = as.numeric(performance(ROCRpredTest, "auc")@y.values)
What is the AUC of this model on the test set? --> 0.7994792 --> 80% OOS accuracy of prediction a poor vs. good case

 Interpretation: Given a random patient from the dataset who ACTUALLY received POOR care + a random patient who 
   ACTUALLY received GOOD care, the AUC = the % of time our model will classify which is which correctly.

We built an expert-trained model by a physician that can accurately ID diabetic patients receiving low quality care.
 - We have observed that the OOS accuracy of the model was 78%.
auc = as.numeric(performance(ROCRpred, "auc")@y.values) --> 0.7745946
 - But most importantly, the model ID's most patients receiving poor care, which is the major objective in the study.
Logistic regression models provide PROBABILITIES of somebody receiving poor quality care + these probabilities can be
   used to prioritize patients for intervention, a particularly useful outcome from the study.
While the accuracy is reasonably high, 78%, it can be, of course, further improved.
   In that respect, I expect that EMR's + not JUST claims could be used in the future to enhance predictive capability of
     such models.
A model like the one we built can be used to analyze literally millions of records, whereas a human can only accurately
   analyze rather small amounts of info
Clearly such a model allows significantly larger scalability, but it does NOT replace expert judgement.
   However, models provide a way to translate expert judgement to a reproducible, testable prediction methodology that
     has significantly higher scalability
Of course, experts can continuously improve + refine the model, as we have seen.
Finally, + quite importantly, models can integrate assessments of MULTIPLE experts into 1 final, unbiased, + 
   unemotional prediction.
Such methods of combining assessments + combining models is a tool we can use as a way of enhancing + improving 
 quantitative models.
