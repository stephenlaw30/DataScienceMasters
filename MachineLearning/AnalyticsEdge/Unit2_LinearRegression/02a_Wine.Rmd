...
summary(wine)
```
1-variable linear regression model using AGST via linear model lm(DV ~ predictor, datasetToUse) function
model1 <- lm(wine$Price ~ wine$AGST, data = wine)
summary(model1)
see description of function, residuals/error terms, coefficient estimates, error, t-value, p-value, R1, Adj. R2, etc.
See that for each 0.6351 increase in AGST in units of AGST, price will increase by 1 unit of price
AGST is also VERY significant, w/ an "okay" R2 and Adj. R2 (adjusted for  of predictor's used to  of data points in
   the model to create system of checks and balances)
   - as R2 increases w/ each predictor added, Adj. R2 adds a penalization factor if an added predictor does NOT improve the
       model
...
summary(model2)
See that AGST's coefficient has increased, and HarvestRain's is slightly negative, while both are V. SIG.
...
summary(model3)
See that only AGST and HarvestRain are SIG and they are V. SIG., while all coefficient's are very small.
 - R2 and Adj. R2 have increased, but the variables aren't sig.
 - coefficient of 0 = does NOT affect the DV --> REMOVE VARS THAT AREN'T SIGNIFICANTLY DIFF FROM 0
 - Std. Err = measure of how likely it is the coefficient will vary from the estimate value
 - T-value = Estimate/SE
     - larger the ABSV of t-value, the MORE likely the coefficient is SIG
 - Pr(>|t) = how plausible it is that the coefficient is actually 0, given the data used to build the model
...
All coefficient's are very small, except AGST, which has a decent impact on Price here
...
Get correlations of ALL predictor's
cor(wine)
...
So Age and FrancePop were highly CORRELATED, and caused MULTICOLLINEARITY problems in the model
   ...
   - High corr between an predictor + the DV is good b/c we're trying to predict which predictor's DO IN FACT affect the DV

Due to possibilites of multicollinearity, ALWAYS REMOVE 1 predictor AT A TIME (BACKWARDS ELIMINATION)
...
Multicollinearity reminds us that coefficient's are only interpretable IN THE PRESENCE OF OTHER VARIABLES BEING USED.
 - High correlations can even cause coefficients to have an unintuitive sign.

Do we have any other highly-correlated predictor's?
...
cor(wine)
Doesn't look like we have any other highly-correlated predictor's
So we'll stick w/ model, using AGST, HarvestRain, WinterRain, and Age as predictor's
...
For this particular application, Bordeaux wine BUYERS PROFIT from being able to PREDICT QUALITY of a wine years
 BEFORE IT MATURES
They know the values of the predictor's (age + weather), don't know the price the wine will eventually sell for.
...
As we add more predictor's our Test Set's R2 does not always increase (can even be negative b/c our model can never
