#tables in R are stored in matrices # they must all be same type like vectors
...
t(FieldGoals)
?matplot
#plot transposed FG matrix as line AND point graph (type = b = both), w/ 4 S=compatible vector symbols (pch),
# and then use colors 1-4 and 6 for the player colors (players = mix of shape + color) 
# and use same shapes + colors for legend as in plot
...
#see drops in minutes due to injuries b/c not normalizing by # of games played
...
lebronPlot <- function(){
  Data <- MinutesPlayed[3,,drop=FALSE]
  matplot(t(Data), type="b", pch=15:18, col=(c(1:4,6)))
  legend("bottomleft", inset = 0.01, legend = Players[3], pch=15:18, col=(c(1:4,6)))
}

lebronPlot()

playersPlot <- function(rows){
...
defCustomPlot(Points)

defCustomPlot(Games)


#ANALYSIS AND INSIGHTS
defCustomPlot(Salary)
' Kobe has the highest peak salary of about $3M per year in 2009, with a steady rise until a drop in 2010 back closer to 
    $2M'
defCustomPlot(Salary/Games)
' Kobe and Rose have higher Salaries/Game b/c of their injuries so they played less games. Need to normalize somehow, but 
    they still may counfound/skew data, so this salary per game or per field goal data may not be the best data combos to 
    look at until we figure out a way to ignore injuries'

defCustomPlot(MinutesPlayed)
  ' again see Kobe and Rose minutes drop in 09, while rose also dropped in 08 and 03, while all players drop in 07'

defCustomPlot(Points)
  ' def need to remove all injuries b/c they are affectinga all in-game stats'
  
#NORMALIZE IN-GAME METRICS BY IN-GAME METRICS
defCustomPlot(FieldGoals/Games)
  ' spread difference is not as bad (mitigate effects a bit), but they are still affected (kobe in 09), but kobe has 
      dropped over the years'
  
defCustomPlot(FieldGoals/FieldGoalAttempts)
  'howard has highest accuracy, while kobe, lebron, and others have dropped'

defCustomPlot(FieldGoalAttempts/Games)
  'howard also has lowest FGA, so he does not take a lot of shots, so does not miss a lot'
  
defCustomPlot(Points/Games)
  'see howard has low ppg b/c little FG made + FGA'
  
defCustomPlot(MinutesPlayed/Games)
  ' why are everyones MPG going down?'
  
defCustomPlot(Games)
  ' games is consistent, why MPG going down?'
  
defCustomPlot(FieldGoals/MinutesPlayed)

defCustomPlot(Points/FieldGoals)
  ' Durant has highest and was growing (due to 3FG or FT?), but some points per FG > 3, so we ahve to remove FTs 
  somehow, while bosh drops'