...
**<li> 50\% of houses in Ames are smaller than 1,499.69 square feet. </li>** --> this is mean, we need median
...
samp1 %>%
  ggplot + 
  geom_histogram(aes(area), binwidth = 250)
```

*This distribution is unimodal and right-skewed, like the population was.*
...
```{r loop}
sample_means50 <- ames %>%
                    rep_sample_n(size = 50, reps = 15000, replace = TRUE) %>% # get 15,000 samples of size 50 w/ replacement
                    summarise(x_bar = mean(area)) # get the mean of means
...
sample_means50 %>%
  ggplot + 
  geom_histogram(aes(x_bar), binwidth = 20)
```

*There are 15,000 elements in sample_means50, with 2 variables, the sample number and its mean area. The center of the distribution is the mean here, since our distribution is quite normal. This would be the value 1499.84, which is quite close to the population mean of 1499.69*
...
sample_means_small <- ames %>%
                    rep_sample_n(size = 10, reps = 25, replace = TRUE) %>% # get 15,000 samples of size 50 w/ replacement
                    summarise(x_bar = mean(area)) # get the mean of means
```

*There are 25 observations and each represents a mean area from a sample of size 25*
...
*My best point estimate is around $194,625.40*
...
sample_means50_price <- ames %>%
  ...
  summarise(x.bar_price = mean(price))

# summary stats
sample_means50_price %>%
  summarise(mu = mean(x.bar_price), pop_med = median(x.bar_price), 
            sigma = sd(x.bar_price), pop_iqr = IQR(x.bar_price),
            pop_min = min(x.bar_price), pop_max = max(x.bar_price),
            pop_q1 = quantile(x.bar_price, 0.25),  # first quartile, 25th percentile
            pop_q3 = quantile(x.bar_price, 0.75))  # third quartile, 75th percentile

# histogram
sample_means50_price %>%
  ggplot + 
  geom_histogram(aes(x.bar_price), binwidth = 2500)
```

*This distribution is quite normal, with a measure of center of mean = $180,755.7. This is our best point estimate of the population mean.*
...
*This distribution is a bit more normal, with less variation around the measure of center, which is the mean price, $180,781*
...
# type your code for this Exercise here, and Run Document
sample_means_price_small <- ames %>%
  sample_n(15) %>%
  summarise(x.bar_price = mean(price))
```

*My best point estimate from this sample of the population mean of prices of homes is $159,772.3*
...
# type your code for this Exercise here, and Run Document
sample_means_price_15 <- ames %>%
  rep_sample_n(15,2000,replace = T) %>%
  summarise(x.bar.price = mean(price))

# summary stats
sample_means_price_15 %>%
  summarise(mu = mean(x.bar.price),
            sigma = sd(x.bar.price),
            median = median(x.bar.price),
            q1 = quantile(x.bar.price,.25),
            q3 = quantile(x.bar.price,.75),
            iqr = IQR(x.bar.price),
            min = min(x.bar.price),
            max = max(x.bar.price))

# histogram
ggplot(sample_means_price_15) + 
  geom_histogram(aes(x.bar.price), binwidth = 2500)
```

*This distribution is mostly normal, but with a slight-right skew, and from the sampling distribution's mean of $180,925, we'd estimate the population mean home price is also $180,925*
...
# type your code for this Exercise here, and Run Document
sample_means_price_150 <- ames %>%
  rep_sample_n(150,2000,replace = T) %>%
  summarise(x.bar.price = mean(price))

# summary stats
sample_means_price_150 %>%
  summarise(mu = mean(x.bar.price),
            sigma = sd(x.bar.price),
            median = median(x.bar.price),
            q1 = quantile(x.bar.price,.25),
...
# histogram
ggplot(sample_means_price_150) + 
  geom_histogram(aes(x.bar.price), binwidth = 2500)
```

*This distribution is more normal than the one for sample size of 15, and from the sampling distribution's mean of $180,860.50, we'd estimate the population mean home price is also $180,860.50*
